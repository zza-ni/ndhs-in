...
function useQueryDate() {
  const [date, setDate] = useState(
    () =>
      new URLSearchParams(window.location.search).get("date") || DateToStr(now)
  );
  useEffect(() => {
      ...
      {days.map((label, i) => (
        <React.Fragment key={label}>
          <input
            ...
            onChange={() => onChange(i)}
          />
          <label className={seg.label} htmlFor={`tab${i + 1}`}>
            {label}
          </label>
        </React.Fragment>
      ))}
...
export default function MenuPage() {
  ...
  const fetchWeek = (wednesdayDateObj) => {
    ...
    fetch(jsonPath)
      ...
      .then((data) => {
        setMenuData(data);
        const maxDate = DateToStr(
          getWednesdayOfWeek(
            DateToStr(new Date(now.getTime() + 4 * 24 * 3600000 + 21540000))
          )
        );
      ...
      .catch(() => {
        alert(
          "식단 정보를 불러오는 데 실패했습니다. 잠시 후 다시 시도해주세요."
        );
  ...
  const currentMenu = useMemo(
    () => menuData?.find((m) => m.day === days[selectedTab]),
    [menuData, selectedTab]
  );
  const titleDate = useMemo(
    () =>
      menuData?.[selectedTab]?.date
        ? formatDate(menuData[selectedTab].date)
        : "",
    [menuData, selectedTab]
  );

  const onPrev = () => {
    const a = strToDate(queryDate);
    setQueryDate(DateToStr(new Date(a.setDate(a.getDate() - 7))));
  };
  const onNext = () => {
    const a = strToDate(queryDate);
    setQueryDate(DateToStr(new Date(a.setDate(a.getDate() + 7))));
  ...
  useEffect(() => {
    const onKey = (e) => {
      if (e.key === "ArrowLeft") setSelectedTab((i) => Math.max(0, i - 1));
      else if (e.key === "ArrowRight")
        setSelectedTab((i) => Math.min(6, i + 1));
    };
      ...
      <main className="main-content page-content">
        <div className="header">
          ...
          <p className="today-date">
            {titleDate ? `${titleDate} (${days[selectedTab]})` : ""}
          </p>
          <button
            id="next-week"
            className="week-control"
            onClick={onNext}
            ref={nextBtnRef}
          >
            &#62;
        ...
        <div className="container" id="menu1">
          ...
          <div className="menu-wrapper">
            <MenuBlocks
              menu={currentMenu}
              highlight={
                menuData?.[selectedTab]?.date?.replace(/-/g, "") ===
                  activeMeal.date && selectedTab === activeMeal.dayIndex
                  ? activeMeal.meal
                  : undefined
              }
            />
