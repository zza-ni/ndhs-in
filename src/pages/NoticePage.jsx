...
export default function NoticePage() {
  ...
  // HTML helpers (shared)
  const enhanceHtml = (html, deferImages) =>
    enhanceHtmlUtil(html, { imageBase: IMAGE_BASE, deferImages });

  // 날짜 포맷: 25/1/2 09:30 형태
  ...
  // 초기 데이터 로드: 단일 글이면 해당 글만, 아니면 목록(캐시 우선)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      setLoading(true);
  ...
  // 아코디언 열릴 때: 해당 패널 이미지 하이드레이트 + 부드러운 스크롤
  useEffect(() => {
    if (!openId) return;
  ...
  // 단일 글 모드: 모든 이미지 즉시 하이드레이트 (eager)
  // 목록 모드: 열린 패널만 lazy 하이드레이트
  useEffect(() => {
    if (loading) return;
    const container = scrollRef.current;
    if (!container) return;
    if (postId) {
      ...
      const el = document
        .getElementById(`accordion-item-${openId}`)
        ?.querySelector(".accordion-panel");
      if (el) hydrateImagesInElement(el, { eager: false });
    }
              ...
              .map((it, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`${acc.item} ${
                      postId ? acc.open : isOpen ? acc.open : ""
                    }`}
                  >
                    <button
                    ...
                    >
                      <div
                        className={`${acc.noticeContent} ${styles.noticePanelInner}`}
