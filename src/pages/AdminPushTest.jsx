...
export default function AdminPushTest() {
  ...
  const onSend = async () => {
    ...
    try {
      ...
      setResult(
        res.ok
          ? data?.message || "전송 완료"
          : `오류: ${data?.message || res.statusText}`
      );
    } catch (e) {
  ...
  const TypeSelector = () => (
    ...
    >
      ...
      />
      <label htmlFor="type-all" role="tab" aria-selected={type === "all"}>
        전체
      </label>
      <input
        type="radio"
        ...
        name="push-type"
        checked={type === "fcm"}
        onChange={() => setType("fcm")}
      />
      <label htmlFor="type-fcm" role="tab" aria-selected={type === "fcm"}>
        FCM
      </label>
      {/* 웹푸시 옵션 제거 */}
    </div>
    ...
    <main className="main-content page-content">
      ...
      <div className="container" style={{ display: "grid", gap: 10 }}>
        ...
        <Card style={{ margin: 0 }}>
          ...
          >
            <CardTitle as="h3" style={{ margin: 0 }}>
              전송
            </CardTitle>
            <TypeSelector />
          </CardHeader>
          ...
          >
            ...
            <button
              className={form.btn}
              onClick={onSend}
              disabled={sending || !password || !title || !body}
            >
              {sending ? "전송 중…" : "전송"}
            </button>
          </CardBody>
