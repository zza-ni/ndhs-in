...
export default async function handler(req, res) {
  ...
  try {
    ...
    const docs = tokensSnapshot.empty ? [] : tokensSnapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
    const groups = new Map();
    for (const d of docs) {
      const key = d.deviceId || d.ua || d.token;
      const ts = (d.updatedAt?.toMillis?.() || d.updatedAt || 0) || (d.createdAt?.toMillis?.() || d.createdAt || 0) || 0;
      const cur = groups.get(key);
      if (!cur || ts > cur.ts) {
        groups.set(key, { token: d.token, ts });
      }
    }
    const tokens = Array.from(new Set(Array.from(groups.values()).map((v) => v.token))).filter(Boolean);
  ...
  return res.status(200).json({ message: `전송 완료 (FCM S${summary.fcm.success}/F${summary.fcm.failure}; WebPush 비활성화됨)`, summary, errors: fcmErrors });
  } catch (error) {
